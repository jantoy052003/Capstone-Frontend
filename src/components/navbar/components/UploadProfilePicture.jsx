...
const UploadProfilePicture = ({showForm, hideUploadForm, uploadProfileImage}) => {
    ...
    <div className={`bg-black bg-opacity-20 fixed left-0 right-0 h-screen top-0 flex justify-center items-center z-50 ${showForm}`}>
      <form onSubmit={uploadProfileImage} className="bg-navbar w-80 md:w-1/2 lg:w-1/4 p-4 rounded-md text-white" encType="multipart/form-data">
        ...
        <div className="flex justify-end items-center mt-4">
          <button type="button" className="bg-blue-600 px-4 py-1 rounded-md mr-4 duration-300 transition-all hover:bg-blue-500" onClick={() => hideUploadForm()}>
            Cancel
          </button>
          <input
            type="submit"
            value="Upload"
            className="cursor-pointer bg-green-600 px-4 py-1 rounded-md duration-300 transition-all hover:bg-green-500"
          />
        </div>
      </form>
    </div>
